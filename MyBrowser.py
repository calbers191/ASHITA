...
class MyBrowser():
    def __init__(self, head=True):
        if head == True:
            ...
            self.browser = self.open_headless_browser()
    ...
    def clinvar_missense_nav(self, gene):
        try:
            ...
            self.browser.driver.find_element_by_id('term').send_keys(gene)
    ...
    def hsf_nav(self, dna_change, transcript_id):
        ...
        time.sleep(1)
        self.browser.find_by_value("Proceed to analysis!").click()
        self.browser.driver.execute_script("document.getElementById('innercontent').scrollIntoView();")
